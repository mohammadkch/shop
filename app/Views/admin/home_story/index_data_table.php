    ...
    <div class="overflow-x-auto rounded-2xl border border-gray-100 dark:border-gray-700">
        <table class="w-full text-sm text-right">
            ...
            <tr>
                ...
                <th class="px-5 py-4">آواتار</th>
                <th class="px-5 py-4">عنوان</th>
                <th class="px-5 py-4">نوع</th>
                <th class="px-5 py-4">مدت (ms)</th>
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-all">
                    ...
                    <td class="px-5 py-4">
                        <?php if (!empty($item['avatar'])): ?>
                            <img src="<?= base_url('images/' . $item['avatar']) ?>"
                                 class="w-12 h-12 object-cover rounded-full border cursor-pointer"
                                 onclick="window.open('<?= base_url('images/' . $item['avatar']) ?>', '_blank')"
                                 alt="<?= esc($item['title']) ?>">
                        <?php else: ?>
                    ...
                    <td class="px-5 py-4">
                        <?php if ($item['type'] == 'image'): ?>
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300">تصویر</span>
                        <?php else: ?>
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-800 dark:bg-purple-900/40 dark:text-purple-300">ویدئو</span>
                        <?php endif; ?>
                    </td>
                    <td class="px-5 py-4"><?= $item['duration'] ?? '-' ?></td>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex space-x-2 rtl:space-x-reverse">
                            <a href="<?= site_url(ADMIN_PATH . '/home-story/edit/' . $item['id']) ?>" class="text-primary hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300" title="ویرایش" aria-label="ویرایش">
                            ...
                            <button type="button" title="حذف" aria-label="حذف" class="delete-btn text-blue-600 hover:text-blue-800" data-id="<?= $item['id'] ?>" data-url="<?= site_url(ADMIN_PATH . '/home-story/delete') ?>">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<?php else: ?>
    <div class="alert alert-warning text-center py-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
        ...
        <h5 class="text-lg font-semibold text-gray-800 dark:text-gray-200">استوری‌ای یافت نشد</h5>
        <p class="text-gray-600 dark:text-gray-400">هیچ استوری با جستجوی شما مطابقت ندارد</p>
    </div>
<?php endif; ?>