                        ...
                        <form method="post" action="<?= site_url($form_action) ?>" enctype="multipart/form-data">
                            ...
                            <div class="space-y-4">
                                ...
                                <?php foreach ($imageTypes as $type): ?>
                                    <?php
                                    $existingImage = isset($groupedImages[$type['id']][0]) ? $groupedImages[$type['id']][0] : null;
                                    ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (menu1Select && menu2Select) {
                        ...
                        .then(data => {
                            ...
                            if (data.status === 'success' && data.options) {
                                data.options.forEach(option => {
                                    ...
                                    if (selectedMenu2Id && selectedMenu2Id == option.id) {
                ...
                const initialMenu1Id = '<?= isset($default_menu_1_id) ? $default_menu_1_id : '' ?>';
                const currentMenu2Id = '<?= isset($edit_row['menu_2_id']) ? $edit_row['menu_2_id'] : '' ?>';

                if (initialMenu1Id) {
                    loadMenu2(initialMenu1Id, currentMenu2Id);
                }
