    ...
    <div class="overflow-x-auto rounded-2xl border border-gray-100 dark:border-gray-700">
        <table class="w-full text-sm text-right">
            ...
            <tr>
                ...
                <th class="px-5 py-4">slug منو</th>
                <th class="px-5 py-4">نوع تصویر</th>
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-all">
                    ...
                    <td class="px-5 py-4">
                        <?php if (!empty($item['image_name'])): ?>
                            <img src="<?= base_url($item['path'] . '/' . $item['image_name']) ?>"
                                 class="w-12 h-12 object-cover rounded-lg border cursor-pointer"
                                 onclick="window.open('<?= base_url($item['path'] . '/' . $item['image_name']) ?>', '_blank')"
                                 alt="<?= esc($item['alt']) ?>">
                        <?php else: ?>
                    ...
                    <td class="px-5 py-4"><?= esc($item['menu_2_name'] ?? '-') ?></td>
                    <td class="px-5 py-4"><?= esc($item['menu_name'] ?? '-') ?></td>
                    <td class="px-5 py-4"><?= esc($item['menu_slug'] ?? '-') ?></td>
                    <td class="px-5 py-4">
                        <?= esc($item['type_name'] ?? '-') ?>
                        <?php if (!empty($item['width']) && !empty($item['height'])): ?>
                            <span class="text-xs text-gray-500">(<?= $item['width'] ?>x<?= $item['height'] ?>)</span>
                        <?php endif; ?>
                    </td>
                    <td class="px-5 py-4"><?= esc($item['alt'] ?: '-') ?></td>
                    <td class="px-5 py-4">
                    ...
                    <td class="px-5 py-4">
                        <div class="flex space-x-2">
                            ...
                            <?php if ($item['is_active'] != 1): ?>
                                <button type="button" class="delete-btn text-red-600 hover:text-red-800" data-id="<?= $item['id'] ?>" data-url="<?= site_url('admin/menu3-image/delete') ?>">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<?php else: ?>
    <div class="alert alert-warning text-center py-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
        ...
        <p class="text-gray-600 dark:text-gray-400">هیچ تصویری با جستجوی شما مطابقت ندارد</p>
    </div>
<?php endif; ?>
